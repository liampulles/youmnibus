...
kubectl exec --namespace=youmnibus mongod-0 -c mongod-container -- mongo --eval 'rs.initiate({_id: "MainRepSet", version: 1, members: [ {_id: 0, host: "mongod-0.mongodb-service.youmnibus.svc.cluster.local:27017"}, {_id: 1, host: "mongod-1.mongodb-service.youmnibus.svc.cluster.local:27017"}, {_id: 2, host: "mongod-2.mongodb-service.youmnibus.svc.cluster.local:27017"} ]});'

# Wait a bit until the replica set should have a primary ready
echo "Waiting for the Replica Set to initialise..."
sleep 30
kubectl exec --namespace=youmnibus  mongod-0 -c mongod-container -- mongo --eval 'rs.status();'

# Create the admin user (this will automatically disable the localhost exception)
echo "Creating user: 'main_admin'"
kubectl exec --namespace=youmnibus  mongod-0 -c mongod-container -- mongo --eval 'db.getSiblingDB("admin").createUser({user:"main_admin",pwd:"'"${1}"'",roles:[{role:"root",db:"admin"}]});'
echo
